...
class GCN_framework:
    ...
    def train(self):
        ...
        for data in self.train_loader:
            ...
            output = F.log_softmax(self.model.lin2(output), dim=-1)
    ...
    def __init__(self,dataset,device=None):   
        ...
        class Net(torch.nn.Module):
            def __init__(self, in_channels, out_channels):
                super(Net, self).__init__()
                self.mlp1 = torch.nn.Linear(in_channels, 30)
                self.conv1 = GINConv(self.mlp1, aggr="add")
                self.mlp2 = torch.nn.Linear(30, 30)
                self.conv2 = GINConv(self.mlp2, aggr="add")
                self.mlp3 = torch.nn.Linear(30, 30)
                self.conv3 = GINConv(self.mlp3, aggr="add")

                self.lin1 = Linear(30*3, 30)
                self.lin2 = Linear(30, out_channels)
...
from torch_geometric.nn import DenseGCNConv, dense_diff_pool,DenseGINConv,DenseGraphConv
    ...
    def __init__(self,dataset,max_nodes,device=None):   
        ...
        class GNN(torch.nn.Module):
            def __init__(self, in_channels, hidden_channels, out_channels,lin=True):
                ...
                #self.mlp3 = torch.nn.Linear(hidden_channels, out_channels)
